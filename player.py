...
class Player(pygame.sprite.Sprite):
    def __init__(self, game_mode):
        ...
        if self.game_mode == "Neuroevolution":
            ...
            layer_sizes = [3 * obstacle_number + 3, 15, 2]  # TODO (Design your architecture here by changing the values)
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        features = [player_x/screen_width, player_y/screen_height,
                    1 if self.player_gravity == 'left' else -1]

        for obstacle in obstacles[:obstacle_number]:
            features.extend([(player_x - obstacle['x']) / screen_width,
                             (player_y - obstacle['y'])/screen_height, self.get_distance(player_x, player_y, obstacle, screen_width, screen_height)])

        remaining = (obstacle_number - len(obstacles)
                     ) if len(obstacles) < obstacle_number else 0
        for i in range(remaining):
            features.extend([(player_x) / screen_width,
                             (player_y)/screen_height, self.get_distance(player_x, player_y, {'x': 0, 'y': 0}, screen_width, screen_height)])

        array_features = np.array(features)
        output = self.nn.forward(array_features)
    ...
    def get_distance(self, player_x, player_y, obstacle, screen_width, screen_height):
        distance = pow(pow((obstacle['x'] - player_x) / screen_width, 2) + pow(
            (obstacle['y'] - player_y) / screen_height, 2), 1/2)
        sign = 1 if player_x < obstacle['x'] else -1
